function [b,stats] = constraint_statrobustfit(X,y,wfun,tune,wasnan,addconst,priorw,dowarn,pars)
...
while((iter==0) || any(abs(b-b0) > D*max(abs(b),abs(b0))))
   ...
   b0 = b;
   [b(perm),wxrank] = wfit(y,X(:,perm),w,pars);
end
...
if (nargout>1)
   ...
   if all(w<D | w>1-D)
       ...
       robust_s = statcontrainedrobustsigma(wfun, radj, xrank, max(mad_s,tiny_s), tune, h);
   end
...
switch lower(method)
    case 'lsqlin'
        ...
        d = yw;
        lb = zeros(n,1);
        options = optimoptions(@lsqlin,'FunctionTolerance',1e-6,'Display','none','MaxIteration',50,'algorithm','interior-point');
        b = lsqlin(C,d,[],[],[],[],lb,[],[],options);
    case 'lagrange'
        lambda = pars.lambda;
        b =  pinv(xw'*xw)*(xw'*yw -eye(n,1)*lambda);
    case 'ridge'
        lambda = pars.lambda;
        b =  pinv(xw'*xw-eye(n)*lambda)*(xw'*yw);
end
