function [trust_functions] = map_pv_errors2(PV,PV_est,t,location,threshold,quant)
...
for i=1:size(PV_est,2)
    ...
    trust_functions{i} = @(az,el) trust_threshold(RF{i},az,el,threshold);
end
...
for i=1:size(PV_est,2)
    ...
    trusted = reshape(trust_functions{i}(X(:),Y(:)),size(X,1),[]);
