function varargout = constraint_robustfit(X,y,pars,wfun,tune,const,priorw,dowarn)
...
if (nargin<4 || isempty(wfun)), wfun = 'bisquare'; end
if nargin<5, tune = []; end
[wfun,tune] = constraint_statrobustwfun(wfun,tune);
if (nargin<6), const='on'; end
switch(const)
 case {'on' 1},  doconst = 1;
...
[varargout{:}] = constraint_statrobustfit(X,y,wfun,tune,wasnan,doconst,priorw,dowarn,pars);
